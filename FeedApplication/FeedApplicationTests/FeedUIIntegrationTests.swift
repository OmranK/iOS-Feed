...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_feedImageView_doesNotRenderLoadedImageWhenNotVisibleAnymore() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        loader.completeLoadingFeed(with: [makeImage()])
        
        let view = sut.simulateNotVisibleImageView(at: 0)
        loader.completeImageLoading(with: anyImageData())
        XCTAssertNil(view?.renderedImage, "Expected cell view to contain no rendered image when the cell is not visible anymore.")
    }
    ...
    func test_loadImageDataCompletion_dispatchesFromBackgroundToMainThread() {
        ...
        sut.loadViewIfNeeded()
        loader.completeLoadingFeed(with: [makeImage()])
        _ = sut.simulateVisibleImageView(at: 0)
    ...
    func test_loadFeedCompletion_rendersErrorMessageOnErrorUntilReload() {
        let (sut, loader) = makeSUT()
...
//        let sut = FeedUIComposer.feedControllerComposedWith(feedLoader: loader, imageLoader: loader)
        ...
        // Combine Alternative to composition
        let sut = FeedUIComposer.feedControllerComposedWith(
